...
import type { DigitalProductCategory } from "@/lib/types";

export default function DigitalProductCategoriesPage() {
  ...
  async function fetchCategories() {
    setLoading(true);
    try {
      const res = await getDigitalProductCategories();
  ...
  function openEdit(cat: DigitalProductCategory) {
  ...
  async function handleSave() {
    ...
    try {
      if (editingId) {
        await updateDigitalProductCategory(editingId, form);
      } else {
        await createDigitalProductCategory(form);
      }
      setShowModal(false);
      fetchCategories();
    } catch (err) {
  ...
  async function handleDelete(cat: DigitalProductCategory) {
    ...
    try {
      await deleteDigitalProductCategory(cat.id);
      fetchCategories();
    } catch (err) {
      console.error(err);
    }
  ...
  return (
    <>
      <AdminHeader
        title="Kategorije digitalnih proizvoda"
        description="Upravljanje kategorijama digitalnih proizvoda"
            ...
            {categories.length === 0 && (
              <tr>
                <td colSpan={6} className="px-6 py-12 text-center text-sm text-gray-500">
                  Nema kategorija digitalnih proizvoda
                </td>
              </tr>
            )}
          </tbody>
      ...
      <Modal
        ...
        title={editingId ? "Izmeni kategoriju" : "Nova kategorija digitalnih proizvoda"}
      >
