...
import { getDigitalOrders } from "@/lib/api/admin";
import { formatPrice, formatDate } from "@/lib/utils";
import type { DigitalOrderListItem } from "@/lib/types";
...
export default function DigitalOrdersPage() {
  ...
  const fetchOrders = useCallback(async () => {
    setLoading(true);
    try {
      const res = await getDigitalOrders({
        page,
        pageSize: 20,
        search,
        status: statusFilter,
  ...
  const columns: Column<DigitalOrderListItem>[] = [
    {
      key: "id",
      header: "ID",
      render: (o) => (
    ...
    },
    {
      key: "product",
      header: "Proizvod",
      render: (o) => <span className="text-gray-900">{o.productName}</span>,
    },
    {
      key: "fileType",
      header: "Tip fajla",
      render: (o) => (
        <Badge variant="default">{o.fileType.toUpperCase()}</Badge>
      ),
    },
    {
      key: "price",
  ...
  return (
    <>
      <AdminHeader
        title="Digitalne narudzbine"
        description="Sve digitalne narudzbine na platformi"
      />
      ...
      <DataTable
        ...
        loading={loading}
        keyExtractor={(o) => o.id}
        onRowClick={(o) => router.push(`/digitalne-narudzbine/${o.id}`)}
        page={page}
        totalPages={totalPages}
        onPageChange={setPage}
        emptyMessage="Nema digitalnih narudzbina"
      />
    </>
  );
}
